...
### 1. 刪除 2026-06-17 的既有資料
- 已從本機資料庫刪除 `2026-06-17` 的資料：
  - `daily_summary`：1 筆
  - `daily_stock_records`：6 筆
- 刪除後已確認該日期資料為 0 筆。
...
- 可指定日期後一鍵刪除當天資料。
- 刪除範圍包含：
  - `daily_summary` 當日總覽資料
  - `daily_stock_records` 當日全部商品資料

### 3. 後端 API 與共用邏輯
- 新增 API：`POST /api/delete-by-date`
- 新增共用日期驗證函式：`validate_record_date()`
- 新增刪除函式：`delete_record_date()`
- 既有 `POST /api/save` 也改為共用日期驗證邏輯，避免重複程式碼。

### 4. 刪除後自動重算日增減損益
- 每次刪除指定日期資料後，會重新執行 `recalc_daily_profit_change()`。
- 可確保各商品後續日期的 `daily_profit_change` 維持正確。

### 5. 前端互動行為
- 刪除前會跳出確認提示，避免誤刪。
- 刪除成功後會提示刪除筆數。
- 若刪除的是目前上傳頁所選日期，畫面中的暫存總覽與表格會一併清空。
- 刪除後會自動刷新：
  - 商品下拉選單
  - 總投資頁資料
  - 區間清單頁資料
...
### 6. 影響檔案
- `app.py`（新增刪除 UI、前端刪除流程、刪除 API、日期驗證與重算邏輯）

### 7. 驗證紀錄
- `python3 -c "import app; print(app.delete_record_date('2026-06-17'))"` 執行成功。
- `python3 -m py_compile app.py` 通過。
- 已確認刪除後資料庫中 `2026-06-17` 無殘留資料。
